    ...
    <div className="min-h-screen bg-gradient-to-b from-white via-slate-50 to-white text-slate-900">
      ...
      <main className="pb-32">
        <div className="container mx-auto px-6 pt-28 sm:pt-32">
          <section className="relative overflow-hidden rounded-[48px] border border-primary/10 bg-white px-6 py-16 sm:px-12 lg:px-16 shadow-[0_40px_120px_rgba(49,41,124,0.12)]">
            <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_left,_rgba(183,166,214,0.45),_transparent_55%),radial-gradient(circle_at_bottom_right,_rgba(49,41,124,0.3),_transparent_55%)]" />
            <div className="absolute -left-32 top-1/2 hidden h-[520px] w-[520px] -translate-y-1/2 rounded-full bg-accent/50 blur-3xl lg:block" />
            <div className="relative z-10 grid items-center gap-12 lg:grid-cols-[1.05fr_0.95fr]">
              <div className="space-y-6">
                ...
                <div className="inline-flex items-center gap-3 rounded-full border border-primary/20 bg-primary/10 px-4 py-2 text-xs font-semibold uppercase tracking-[0.35em] text-primary">
                  {post.category?.name || 'Time Study'}
                </div>
                <h1 className="text-4xl font-semibold leading-tight tracking-tight text-primary-dark sm:text-5xl lg:text-6xl">{post.title}</h1>
          ...
          <section className="relative -mt-24 lg:-mt-32">
            <div className="mx-auto max-w-5xl rounded-[40px] border border-slate-200 bg-white p-8 text-slate-900 shadow-[0_40px_140px_rgba(49,41,124,0.12)] sm:p-10 lg:p-14">
              <div className="grid gap-6 sm:grid-cols-3">
                <div className="rounded-3xl border border-primary/10 bg-primary/5 p-6">
                  <p className="text-xs font-semibold uppercase tracking-[0.35em] text-primary/80">Published</p>
                  <p className="mt-3 text-lg font-semibold text-primary-dark">{publishedAt}</p>
                </div>
                <div className="rounded-3xl border border-primary/10 bg-primary/5 p-6">
                  <p className="text-xs font-semibold uppercase tracking-[0.35em] text-primary/80">Author</p>
                  <p className="mt-3 text-lg font-semibold text-primary-dark">{post.author?.name || 'Meteorroids Crew'}</p>
                </div>
                <div className="rounded-3xl border border-primary/10 bg-primary/5 p-6">
                  <p className="text-xs font-semibold uppercase tracking-[0.35em] text-primary/80">Cadence</p>
                  <p className="mt-3 text-lg font-semibold text-primary-dark">{readingMinutes} minute read</p>
                </div>
              </div>
                  ...
                  {post.tags?.length ? (
                    <div className="flex flex-wrap items-center gap-3">
                      ...
                      <div className="flex flex-wrap gap-2 text-sm text-primary">
                        {post.tags.map((tag) => (
                          <span key={tag.id} className="rounded-full border border-primary/30 px-3 py-1">#{tag.slug}</span>
                        ))}
                      </div>
                    </div>
                  ) : null}
